"use client";

import Link from "next/link";
import { motion } from "framer-motion";
import { Calendar, User, Clock, ArrowLeft } from "lucide-react";
import type { BlogPost } from "@/lib/data";

const categoryColors: Record<string, string> = {
  JEE: "bg-indigo/20 text-indigo-light",
  NEET: "bg-green-500/20 text-green-400",
  NDA: "bg-amber-500/20 text-amber-400",
  "Study Tips": "bg-purple-500/20 text-purple-400",
};

function renderMarkdown(content: string) {
  const lines = content.split("\n");
  const elements: React.ReactNode[] = [];
  let inList = false;
  let listItems: string[] = [];

  const flushList = () => {
    if (listItems.length > 0) {
      elements.push(
        <ul key={`list-${elements.length}`} className="space-y-2 my-4 pl-4">
          {listItems.map((item, i) => (
            <li key={i} className="flex items-start gap-2 text-slate-300 text-sm leading-relaxed">
              <span className="w-1.5 h-1.5 rounded-full bg-indigo-light mt-2 shrink-0" />
              <span dangerouslySetInnerHTML={{ __html: item.replace(/\*\*(.*?)\*\*/g, '<strong class="text-white font-semibold">$1</strong>') }} />
            </li>
          ))}
        </ul>
      );
      listItems = [];
    }
    inList = false;
  };

  lines.forEach((line, i) => {
    const trimmed = line.trim();

    if (trimmed.startsWith("- ")) {
      inList = true;
      listItems.push(trimmed.slice(2));
      return;
    }

    if (inList) flushList();

    if (trimmed.startsWith("### ")) {
      elements.push(
        <h3 key={i} className="font-sora font-bold text-xl text-white mt-8 mb-3">
          {trimmed.slice(4)}
        </h3>
      );
    } else if (trimmed.startsWith("## ")) {
      elements.push(
        <h2 key={i} className="font-sora font-bold text-2xl text-white mt-10 mb-4">
          {trimmed.slice(3)}
        </h2>
      );
    } else if (trimmed === "") {
      // skip empty
    } else {
      elements.push(
        <p
          key={i}
          className="text-slate-300 text-sm md:text-base leading-relaxed mb-4"
          dangerouslySetInnerHTML={{
            __html: trimmed.replace(
              /\*\*(.*?)\*\*/g,
              '<strong class="text-white font-semibold">$1</strong>'
            ),
          }}
        />
      );
    }
  });

  if (inList) flushList();
  return elements;
}

export function BlogPostClient({
  post,
  relatedPosts,
}: {
  post: BlogPost;
  relatedPosts: BlogPost[];
}) {
  const shareUrl = typeof window !== "undefined" ? window.location.href : "";

  return (
    <div className="pt-24 md:pt-28 pb-16 md:pb-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Back Link */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="mb-8"
        >
          <Link
            href="/blogs"
            className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-white transition-colors"
          >
            <ArrowLeft size={16} />
            Back to Blog
          </Link>
        </motion.div>

        {/* Hero */}
        <motion.header
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className="max-w-3xl mb-10 md:mb-12"
        >
          <span
            className={`inline-block px-3 py-1 rounded-md text-[11px] font-semibold uppercase tracking-wider mb-4 ${categoryColors[post.category] || "bg-indigo/20 text-indigo-light"}`}
          >
            {post.category}
          </span>
          <h1 className="font-sora font-extrabold text-3xl md:text-4xl leading-tight mb-5">
            {post.title}
          </h1>
          <div className="flex flex-wrap items-center gap-4 text-sm text-slate-400">
            <span className="flex items-center gap-1.5">
              <User size={14} />
              {post.author}
            </span>
            <span className="flex items-center gap-1.5">
              <Calendar size={14} />
              {new Date(post.date).toLocaleDateString("en-IN", {
                day: "numeric",
                month: "long",
                year: "numeric",
              })}
            </span>
            <span className="flex items-center gap-1.5">
              <Clock size={14} />
              {post.readTime}
            </span>
          </div>
        </motion.header>

        {/* Content + Sidebar */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10">
          {/* Main Content */}
          <motion.article
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ duration: 0.6, delay: 0.2 }}
            className="lg:col-span-8"
          >
            <div className="glass-card rounded-lg p-6 md:p-8">
              {renderMarkdown(post.content)}
            </div>
          </motion.article>

          {/* Sidebar */}
          <aside className="lg:col-span-4">
            <div className="lg:sticky lg:top-24 space-y-6">
              {/* Related Posts */}
              {relatedPosts.length > 0 && (
                <motion.div
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ duration: 0.5, delay: 0.3 }}
                  className="glass-card rounded-lg p-6"
                >
                  <h3 className="font-sora font-bold text-sm uppercase tracking-wider text-slate-300 mb-4">
                    Related Posts
                  </h3>
                  <div className="space-y-4">
                    {relatedPosts.map((rp) => (
                      <Link
                        key={rp.slug}
                        href={`/blogs/${rp.slug}`}
                        className="block group"
                      >
                        <h4 className="text-sm font-medium text-slate-300 group-hover:text-indigo-light transition-colors leading-snug mb-1">
                          {rp.title}
                        </h4>
                        <p className="text-xs text-slate-500">
                          {rp.readTime} • {rp.author}
                        </p>
                      </Link>
                    ))}
                  </div>
                </motion.div>
              )}

              {/* Share */}
              <motion.div
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.5, delay: 0.4 }}
                className="glass-card rounded-lg p-6"
              >
                <h3 className="font-sora font-bold text-sm uppercase tracking-wider text-slate-300 mb-4">
                  Share This Post
                </h3>
                <div className="flex gap-3">
                  {[
                    {
                      label: "Twitter",
                      href: `https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(post.title)}`,
                      svg: "M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z",
                    },
                    {
                      label: "LinkedIn",
                      href: `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(shareUrl)}`,
                      svg: "M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z",
                    },
                    {
                      label: "WhatsApp",
                      href: `https://wa.me/?text=${encodeURIComponent(post.title + " " + shareUrl)}`,
                      svg: "M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z",
                    },
                  ].map((social) => (
                    <a
                      key={social.label}
                      href={social.href}
                      target="_blank"
                      rel="noopener noreferrer"
                      aria-label={`Share on ${social.label}`}
                      className="w-10 h-10 rounded-lg bg-white/5 hover:bg-indigo/20 flex items-center justify-center transition-all group"
                    >
                      <svg className="w-4 h-4 fill-slate-400 group-hover:fill-white transition-colors" viewBox="0 0 24 24">
                        <path d={social.svg} />
                      </svg>
                    </a>
                  ))}
                </div>
              </motion.div>

              {/* CTA */}
              <motion.div
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ duration: 0.5, delay: 0.5 }}
                className="glass-card rounded-lg p-6 text-center"
              >
                <h3 className="font-sora font-bold text-lg mb-2">
                  Ready to Start Preparing?
                </h3>
                <p className="text-slate-400 text-sm mb-4">
                  Try Rank Sarthi free today.
                </p>
                <Link
                  href="/#pricing"
                  className="inline-block px-6 py-2.5 bg-saffron hover:bg-saffron-light text-navy font-semibold text-sm rounded-lg transition-all shadow-lg shadow-saffron/20"
                >
                  Start Free Test
                </Link>
              </motion.div>
            </div>
          </aside>
        </div>
      </div>
    </div>
  );
}
